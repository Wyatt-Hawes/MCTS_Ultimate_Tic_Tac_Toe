...
def rollout(board, state):
    ...
    while board.is_ended(state) == False:
        ...
        #Checks to see if it can make 3 in a row or block a 3 in a row, otherwise returns random
        smart_choice = check_for_box_increase(board,state)
        
        
        
        state = board.next_state(state,smart_choice)
...
def think(board, state):
    ...
    #Using timer instead of number of nodes
    start = time()                 
    # This is in seconds.
    while (time() - start) < 1:
    ...
    # estimated win rate.
    #return None
    print("MCTS_modified calculated move ", best_move, ":%.0f"%(best_move_ratio * 100) + "%", "(",no_child_nodes,")")
    return best_move

def get_owned_boxes(board,state,player):
    player_owned = 0
    
    #Dictionary of (row,col) keys, values either 1, 2, or 0 for player ownership
    owned_boxes = board.owned_boxes(state)
    
    for i in range(3):
        for j in range(3):
            if owned_boxes[(i,j)] == player:
                player_owned += 1
    
    return player_owned

def check_for_box_increase(board,state):
    
    owned = get_owned_boxes(board,state, state[-1])
    
    legal_actions = board.legal_actions(state)
    L = len(legal_actions)
    if (L >= 7 and L <= 9) or (L > 76):
        return choice(board.legal_actions(state))
    
    for move in legal_actions:
        new_owned = get_owned_boxes(board, board.next_state(state,move), state[-1])
        
        #Other player can never gain a box on a move you make
        if owned != new_owned:
            #take move, can connect 3
            #print("3 match detected, choosing that move")
            return move
    ...
    other_player_score =  get_owned_boxes(board,state,other_player)
    
    opposing_state = list(state)
    opposing_state[-1] = other_player
    opposing_state = tuple(opposing_state)
    
    
    for move in board.legal_actions(opposing_state):
        new_owned = get_owned_boxes(board, board.next_state(opposing_state,move), other_player)
        
        #Other player can never gain a box on a move you make
        if other_player_score != new_owned:
            #take move, can connect 3
            #print("Block, choosing that move")
            return move

    #print("No match or block, returning random")
    return choice(board.legal_actions(state))